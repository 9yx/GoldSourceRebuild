...
//From Source's checksum_crc.cpp - Solokiller
extern "C"
{
...
int CRC_File( CRC32_t* crcvalue, char* pszFileName )
{
	...
	while( uiBytesLeft > 0 )
	{
		iRead = FS_Read( chunk, min( ARRAYSIZE( chunk ), uiBytesLeft ), hFile );
...
{
	FileHandle_t hFile = FS_Open( pszFileName, "rb" );

	if( !hFile )
		return false;

	FS_Size( hFile );
	int startOfs = FS_Tell( hFile );

	dheader_t header;
	if( FS_Read( &header, sizeof( header ), hFile ) != sizeof( header ) )
	{
	...
	auto iBSPVersion = LittleLong( header.version );

	//Supports Quake BSP & Half-Life BSP
	if( iBSPVersion != 29 && iBSPVersion != BSPVERSION )
	{
		FS_Close( hFile );
	...
	const int nLumpID = stricmp( com_gamedir, "bshift" ) ? LUMP_PLANES : LUMP_ENTITIES;
	...
	{
		//Ignore the entity data lump so ripent doesn't invalidate CRCs.
		if( nLumpID == i )
			continue;

		const auto& lump = header.lumps[ i ];

		FS_Seek( hFile, startOfs + lump.fileofs, FILESYSTEM_SEEK_HEAD );

		if( lump.filelen )
		{
			int iBytesLeft = lump.filelen;

			while( iBytesLeft > 0 )
			{
				const auto iBytesRead = FS_Read( chunk, min( static_cast<int>( ARRAYSIZE( chunk ) ), iBytesLeft ), hFile );

				if( iBytesRead > 0 )
				{
					iBytesLeft -= iBytesRead;
					CRC32_ProcessBuffer( crcvalue, chunk, iBytesRead );
				}
...
BOOL MD5_Hash_File( unsigned char digest[ 16 ], char *pszFileName, int bUsefopen, int bSeed, unsigned int seed[ 4 ] )
{
	...
	if( uiSize > 0 )
	{
		...
		while( 1 )
		{
			const auto iBytesRead = FS_Read( chunk, min( ARRAYSIZE( chunk ), uiSize ), hFile );
